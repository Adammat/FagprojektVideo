			...
			for(int l=0;l<pixels_block;l++){
				...
				for(int y=0;y<block_size;y++){
					for(int x=0;x<block_size;x++){
						sum+= cos((((2*x)+1)*(u*M_PI))/(2*block_size))*cos((((2*x)+1)*(u*M_PI))/(2*block_size))*buffer[((j+(i*block_width))*64)+x+(y*8)];
					}
				}
				...
				output[(((j+(i*block_width))*pixels_block))+(backward_DCT_path[l])]= (int) round((multiplier*sum)/compression_product);
				if(i==0 && j==0 && l==63){
